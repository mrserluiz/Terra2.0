...
public class NMSChunkGeneratorDelegate extends ChunkGenerator {
   ...
   public CompletableFuture<ChunkAccess> fillFromNoise(
      @NotNull Blender blender, @NotNull RandomState noiseConfig, @NotNull StructureManager structureAccessor, @NotNull ChunkAccess chunk
   ) {
         ...
         for (int z = 0; z < 16; z++) {
            ...
            for (int y = world.getMaxHeight(); y >= world.getMinHeight(); y--) {
               ...
               if (noise > threshold) {
                  chunk.setBlockState(
                     new BlockPos(x, y, z),
                     ((CraftBlockData)((BukkitBlockState)this.delegate
                              .getPalette(x + xi, y, z + zi, world, biomeProvider)
                              .get(depth, x + xi, y, z + zi, world.getSeed()))
                           .getHandle())
                        .getState(),
                     0
                  );
   ...
   public int getBaseHeight(int x, int z, @NotNull Types heightmap, @NotNull LevelHeightAccessor world, @NotNull RandomState noiseConfig) {
      ...
      while (
         y >= this.getMinY()
            && !heightmap.isOpaque().test(((CraftBlockData)this.delegate.getBlock(properties, x, y - 1, z, biomeProvider).getHandle()).getState())
      ) {
         y--;
      }
