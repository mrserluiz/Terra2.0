...
public class FileFilterUtils {
   ...
   public static File[] filter(IOFileFilter filter, File... files) {
      ...
      return files == null ? FileUtils.EMPTY_FILE_ARRAY : filterFiles(filter, Stream.of(files), Collectors.toList()).toArray(FileUtils.EMPTY_FILE_ARRAY);
   }
   ...
   private static <R, A> R filterFiles(IOFileFilter filter, Stream<File> stream, Collector<? super File, A, R> collector) {
      ...
      return stream == null ? Stream.<File>empty().collect(collector) : stream.filter(filter::accept).collect(collector);
   }
   ...
   public static List<File> filterList(IOFileFilter filter, Iterable<File> files) {
      return files == null ? Collections.emptyList() : filterFiles(filter, StreamSupport.stream(files.spliterator(), false), Collectors.toList());
   }
   ...
   public static Set<File> filterSet(IOFileFilter filter, Iterable<File> files) {
      return files == null ? Collections.emptySet() : filterFiles(filter, StreamSupport.stream(files.spliterator(), false), Collectors.toSet());
   }
