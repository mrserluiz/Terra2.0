...
import com.dfsek.terra.lib.google.common.primitives.Doubles;
   ...
   private void merge(long otherCount, double otherMean, double otherSumOfSquaresOfDeltas, double otherMin, double otherMax) {
      if (this.count == 0L) {
         ...
         if (Doubles.isFinite(this.mean) && Doubles.isFinite(otherMean)) {
            ...
            this.mean = this.mean + delta * otherCount / this.count;
            this.sumOfSquaresOfDeltas = this.sumOfSquaresOfDeltas + (otherSumOfSquaresOfDeltas + delta * (otherMean - this.mean) * otherCount);
         } else {
