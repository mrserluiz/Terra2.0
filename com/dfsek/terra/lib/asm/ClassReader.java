...
public class ClassReader {
   ...
   private int readMethod(ClassVisitor classVisitor, Context context, int methodInfoOffset) {
      ...
      if (methodVisitor instanceof MethodWriter) {
         ...
         if (methodWriter.canCopyMethodAttributes(
            this, synthetic, (context.currentMethodAccessFlags & 131072) != 0, this.readUnsignedShort(methodInfoOffset + 4), signatureIndex, exceptionsOffset
         )) {
      ...
      while (attributesCount-- > 0) {
         ...
         if ("LocalVariableTable".equals(attributeName)) {
            if ((context.parsingOptions & 2) == 0) {
               ...
               for (int var70 = currentLocalVariableTableOffset + 2; localVariableTableLength-- > 0; var70 += 10) {
                  int startPc = this.readUnsignedShort(var70);
                  this.createDebugLabel(startPc, labels);
                  int length = this.readUnsignedShort(var70 + 2);
      ...
      while (currentOffset < bytecodeEndOffset) {
         ...
         for (;
            ...
            if (currentInvisibleTypeAnnotationBytecodeOffset == currentBytecodeOffset) {
               ...
               currentAnnotationOffset += 2;
               this.readElementValues(
                  methodVisitor.visitInsnAnnotation(context.currentTypeAnnotationTarget, context.currentTypeAnnotationTargetPath, annotationDescriptor, false),
                  currentAnnotationOffset,
                  true,
                  charBuffer
               );
   ...
   private int readVerificationTypeInfo(int verificationTypeInfoOffset, Object[] frame, int index, char[] charBuffer, Label[] labels) {
      ...
      int tag = this.classFileBuffer[currentOffset++] & 255;
      switch (tag) {
         case 0:
   ...
   public short readShort(int offset) {
      byte[] classBuffer = this.classFileBuffer;
      return (short)((classBuffer[offset] & 255) << 8 | classBuffer[offset + 1] & 0xFF);
   }
