...
final class Constants {
   ...
   static boolean isWhitelisted(String internalName) {
      if (!internalName.startsWith("com/dfsek/terra/lib/asm/")) {
         return false;
      }

      String member = "(Annotation|Class|Field|Method|Module|RecordComponent|Signature)";
      return internalName.contains("Test$")
         || Pattern.matches("com/dfsek/terra/lib/asm/util/Trace" + member + "Visitor(\\$.*)?", internalName)
         || Pattern.matches("com/dfsek/terra/lib/asm/util/Check" + member + "Adapter(\\$.*)?", internalName);
   }
